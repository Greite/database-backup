...
// chownTreeAs is only ever called from DropAndReexec, which is linux-only.

// chownTreeAs hands the whole backup tree to the unprivileged uid/gid.
// v1 ran as root and left root-owned job directories behind; without
// this, migrated deployments silently stop producing backups. It also
// re-owns the tree when PUID/PGID change between runs.
//
// It uses os.Lchown rather than os.Chown so a symlink entry has the link
// itself re-owned instead of its target. It runs as root during the
// startup phase, before privileges are dropped, and /backups is a
...
// but it could only re-own a file to the unprivileged uid/gid (not to
